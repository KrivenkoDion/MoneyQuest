...
function Achievements() {
  ...
  const claimQuest = async (questId: string) => {
    ...
    const data = await res.json();
    if (data.xp_reward) {
      showMsg(`+${data.xp_reward} XP claimed! 🎉`);
      loadQuests();
      loadShop(); // refresh XP
    } else {
  ...
  const s: Record<string, any> = {
    ...
    questDesc: { margin: "0 0 14px", fontSize: 13, color: "#999" },
    claimBtn: {
      ...
      {tab === "quests" && quests.map((q) => (
        <div key={q.id} style={s.card}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ flex: 1 }}>
              ...
              <span style={{
                ...
                fontSize: 12, color: "#c9a84c", fontWeight: 600, marginBottom: 14,
              }}>
                +{q.xp_reward} XP
              </span>
            </div>
          ...
          {q.claimed ? (
            ...
            <button style={s.claimBtn} onClick={() => claimQuest(q.id)}>Claim +{q.xp_reward} XP</button>
          ) : (
      ...
      {tab === "shop" && (
        <>
          <div style={{ margin: "0 24px 16px", padding: "14px 20px", background: "white", borderRadius: 16 }}>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 24, fontWeight: 800, color: "#1a1a2e" }}>⭐ {xp} XP</p>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, margin: "0 24px" }}>
            {items.map((item) => (
              <div key={item.id} style={{ ...s.card, margin: 0, textAlign: "center" as const }}>
                ...
                <p style={s.itemCost}>⭐ {item.cost} XP</p>
                {item.owned ? (
                  <button style={s.ownedBtn} disabled>Owned</button>
                ) : (
                  <button
                    style={{ ...s.buyBtn, background: xp >= item.cost ? "#1a1a2e" : "#e8e8e0", color: xp >= item.cost ? "white" : "#999", cursor: xp >= item.cost ? "pointer" : "default" }}
                    onClick={() => xp >= item.cost && buyItem(item.id)}
                  >
                    {xp >= item.cost ? "Buy" : "Need more XP"}
                  </button>
                )}
              </div>
            ))}
